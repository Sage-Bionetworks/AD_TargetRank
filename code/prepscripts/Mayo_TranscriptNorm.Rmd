...
source('~/AD_TargetRank/utilityFunctions/parallelDuplicateCorrelation.R')
source('~/AD_TargetRank/utilityFunctions/irwsva.build.R')
source('~/AD_TargetRank/utilityFunctions/f.pvalue.R')
...
COUNT_ID <- 'syn10499227';
ALL_USED_IDs <- COUNT_ID
COUNT_CBE <- syn_temp$get(COUNT_ID)$path %>%
  read.table(header=T, sep='\t', check.names = F, row.names = 1)

COUNT_ID <- 'syn10507722';
ALL_USED_IDs <- COUNT_ID
COUNT_TCX <- syn_temp$get(COUNT_ID)$path %>%
  read.table(header=T, sep='\t', check.names = F, row.names = 1)

COUNT <- as.data.frame( cbind(COUNT_CBE, COUNT_TCX) )  
...
### Predict missing PMI
```{r predict.pmi}
# Get PMI from the gene level data used to predict
GeneLvlMeta_WithPMI <- fread(syn_temp$get('syn20801992')$path, data.table = F)
row.names(GeneLvlMeta_WithPMI) <- GeneLvlMeta_WithPMI$SampleID
METADATA$PMI <- NA
METADATA[row.names(GeneLvlMeta_WithPMI),]$PMI <-  GeneLvlMeta_WithPMI$PMI 
...
while(length(residualSigCovars$significantCovars)!=0 && loopCount <= 20){
  ...
  if (!is.nan(correlation$cor)){
    ...
    #_# Fix NA coeefcient to stop PCA error
    CQN.GENE_EXPRESSION$E[is.na(CQN.GENE_EXPRESSION$E)] = 0
    VOOM.ADJUSTED.FIT$coefficients[is.na(VOOM.ADJUSTED.FIT$coefficients)] = 0
...
activityDescription = 'Covariate analysis and Regrsison of AD Diagnosis of aligned effective protien coding transcript level counts with CQN normalisation ( TCX, CBE )';
thisFileName <- 'Mayo_TranscriptNorm.Rmd'
# Github link
...
activityName = 'Covariate and Diagnosis Regression';
activityDescription = 'Covariate analysis and Regrsison of AD Diagnosis of aligned effective counts with CQN normalisation (CBE, TCX)';
CODE <- syn_temp$store(synapseclient$Folder(name = "Mayo", parentId = parentId))
#Set Used SynIDs For Provenance
Syns_Used <- c( "syn10499227","syn10507722","syn3817650","syn5223705","syn8698214","syn8698211","syn22087693","syn6126119","syn6126114","syn20801992")
...
createAndKnitToFolderEntityClient(file = "~/AD_TargetRank/code/prepscripts/Mayo_TranscriptNorm.Rmd",
                                          parentId ="syn22092490",
                                          folderName = 'Mayo')
```