...
#Load Gene Set:
Genes <- read.table( paste0(config$filedir, '/genelists/', config$genelistfile ), header=F, sep='\t')
...
if( 2 %in% config$modules ){
  writeLines('Modeling and ranking genelist ', config$genelistfile ,' by Differential Expression evidence')
  
  source('code/modules/02-DiffExp_module.R')
...
if( 3 %in% config$modules ){
  writeLines('Modeling and ranking genelist ', config$genelistfile ,' by Differential Proteomic Expression evidence')
...
if( 4 %in% config$modules ){
  writeLines('Modeling and ranking genelist ', config$genelistfile ,' by eQTL evidence')
...
if( 5 %in% config$modules ){
  writeLines('Modeling and ranking genelist ', config$genelistfile ,' by Coexpression Module evidence')
...
if( 6 %in% config$modules ){
  writeLines('Modeling and ranking genelist ', config$genelistfile ,' by Module Cell-Type Enrichments evidence')
